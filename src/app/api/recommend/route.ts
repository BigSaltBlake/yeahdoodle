...
function getDateRange(timeframe: string): { start: Date; end: Date } {
  ...
  if (timeframe === 'Tonight') {
    const end = new Date(today); end.setHours(23, 59, 59, 999); return { start: now, end }
  }
  ...
  if (timeframe === 'This weekend') {
    ...
    const friday = new Date(today); friday.setDate(friday.getDate() + daysUntilFri)
    const sunday = new Date(friday); sunday.setDate(sunday.getDate() + 2); sunday.setHours(23, 59, 59, 999)
    return { start: (dow === 0 || dow === 6) ? now : friday, end: sunday }
  }
  const end = new Date(today); end.setDate(end.getDate() + 28); return { start: now, end }
}

function getSerpQueriesForCity(timeframe: string, city: string, isLocal: boolean): string[] {
  const when = timeframe === 'Tonight' ? 'tonight' : timeframe === 'Tomorrow' ? 'tomorrow'
    : timeframe === 'This weekend' ? 'this weekend' : 'this month'
  const base = [`events ${when} in ${city}`, `things to do ${when} in ${city}`]
  if (isLocal) base.push(`things to do near ${city}`, `outdoor activities near ${city}`)
  return base
}
        ...
        try {
          ...
          for (let i = 0; i < events.length; i++) {
            ...
            results.push({
              ...
              title, venue_name: venue?.name ?? address?.[0] ?? null, date_start: dateStart,
              ...
              description: (e.description ?? null) as string | null, ai_description: null,
              distanceLabel, source: isActivity ? 'activity' : 'live',
            })
          }
        } catch (err) { console.error('[recommend] SerpAPI error:', err) }
      })
    }),
  )
  return results
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (hasGps) {
      ...
      const regionalDist = regionalGeo ? haversine(lat!, lng!, regionalGeo.resultLat, regionalGeo.resultLng) : null
      geoDisplayName = localGeo?.displayName || regionalGeo?.displayName || 'your area'
      resolvedCity   = resolvedCity || regionalCity || localCity
      if (localCity) cities.push({ name: localCity, distanceLabel: 'Right here', isLocal: true })
      if (regionalCity && regionalCity !== localCity) {
        cities.push({ name: regionalCity, distanceLabel: regionalDist ? driveTimeLabel(regionalDist) : 'Nearby', isLocal: false })
      }
      if (cities.length === 0 && resolvedCity) cities.push({ name: resolvedCity, distanceLabel: 'Nearby', isLocal: false })
    } else if (resolvedCity) {
      cities = [{ name: resolvedCity, distanceLabel: '', isLocal: false }]
    }

    // 2. Parallel fetches
    const liveEventsPromise = cities.length > 0 ? fetchLiveSerpEvents(cities, timeframe) : Promise.resolve([] as EventRow[])

    let dbRowsPromise: Promise<EventRow[]> = Promise.resolve([])
    if (isSupabaseConfigured() && resolvedCity) {
      let q = supabase.from('events').select('*')
        .ilike('city', `%${resolvedCity}%`).eq('is_duplicate', false)
        .gte('date_start', dateStart.toISOString()).lte('date_start', dateEnd.toISOString())
    ...
    if (rows.length < 5 && hasGps && isSupabaseConfigured()) {
      ...
      if (regionalCity && regionalCity !== resolvedCity) {
        const { data: expansion } = await supabase.from('events').select('*')
          .ilike('city', `%${regionalCity}%`).eq('is_duplicate', false)
          .gte('date_start', dateStart.toISOString()).order('date_start', { ascending: true }).limit(40)
    ...
    // 6. Build AI prompt
    const locationLabel = hasGps ? `near your location (${geoDisplayName})` : `in ${resolvedCity}`
    const eventList = rows.map((r, i) => {
      const price = r.price_min === 0 ? 'Free' : r.price_min ? `$${r.price_min}` : 'price unknown'
      const date  = r.date_start ? new Date(r.date_start).toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric', hour: 'numeric' }) : 'Anytime'
    ...
    const timeframeInstruction =
      ...
      'Show variety across the coming weeks.'
    const hasActivities = rows.some(r => r.source === 'activity')
    const activityNote  = hasActivities ? " Some entries are [activity] — timeless things to do (hiking, kayaking, tours). Include these if they match the user's vibe." : ''
...
Pick the 3 BEST events or activities that match this person's vibe. ${timeframeInstruction}${activityNote} Prioritise variety. If the user is in a rural or outdoor area, outdoor activities are valid picks.
  ...
  {"id":"<id>","rank":3,"pitch":"<...>"}
    ...
    if (!anthropicKey) {
      const top3 = rows.slice(0, 3).map((r, i) => ({
        ...
        source: r.source, distanceLabel: r.distanceLabel,
      }))

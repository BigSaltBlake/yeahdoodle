...
function parseGoogleEventDate(dateStr: string | undefined): string | null {
  ...
  try {
    ...
    const d = new Date(cleaned); if (!isNaN(d.getTime())) return d.toISOString()
    const d2 = new Date(`${cleaned} ${new Date().getFullYear()}`); if (!isNaN(d2.getTime())) return d2.toISOString()
  } catch { /* */ }
  return null
}
...
function parseSerpPrice(ticketInfo: Array<{ source?: string; link?: string; price?: string }> | undefined): number | null {
  ...
  for (const t of ticketInfo) {
    ...
    const m = t.price.match(/\$(\d+)/); if (m) return parseInt(m[1], 10)
  }
  return null
}

async function fetchLiveSerpEvents(cityName: string, timeframe = 'Tonight'): Promise<EventRow[]> {
  const serpKey = process.env.SERPAPI_KEY; if (!serpKey) return []
  const queries = getSerpQueries(timeframe, cityName)
  const results: EventRow[] = []; const seen = new Set<string>()
  await Promise.all(queries.map(async (q, qi) => {
    try {
      ...
      const data = await res.json(); const events: unknown[] = data.events_results ?? []
      for (let i = 0; i < events.length; i++) {
        const e = events[i] as Record<string, unknown>
        const title = (e.title as string) ?? 'Local Event'
        const key = title.toLowerCase().slice(0, 40); if (seen.has(key)) continue; seen.add(key)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const liveEventsPromise = (city.trim() || hasGps) ? fetchLiveSerpEvents(city.trim() || 'nearby', timeframe) : Promise.resolve([] as EventRow[])
  ...
  {"id":"<id>","rank":3,"pitch":"<...>"}
    ...
    if (!anthropicKey) {
      ...
      return NextResponse.json({ picks: rows.slice(0, 3).map((r, i) => ({
        ...
        priceFormatted: formatPrice(r.price_min, r.price_max, r.is_free), ticketUrl: r.ticket_url, imageUrl: r.image_url, category: r.category,
      })) })
    }

      ...
      <div className="relative h-36 shrink-0 overflow-hidden bg-white/5">
        ...
        <div className="absolute top-2 left-2 flex gap-1.5 flex-wrap">
          <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
            isNow ? 'bg-green-500 text-white animate-pulse' : 'bg-yd-orange text-white'
          }`}>
            {result.start_label}
          </span>
...
export default function NowMode({ open, onClose }: NowModeProps) {
  ...
  async function fetchResults(t: TimeOption, d: DriveOption, v: VibeOption) {
    ...
    try {
      const res = await fetch('/api/now', {
        ...
        body: JSON.stringify({ lat, lng, time_available: t, max_drive_min: d, vibe: v, location_label: locationLabel }),
      })
  ...
  function handleLetsGo(result: NowResult) {
    ...
    setTimeout(() => { window.open(result.maps_url, '_blank') }, 400)
  }
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      >
        <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-white/8 shrink-0">
          ...
          <button onClick={onClose} className="text-white/40 hover:text-white/80 transition-colors text-xl w-8 h-8 flex items-center justify-center">
            ✕
          </button>
        </div>
        ...
        <div className="overflow-y-auto flex-1 px-5 py-5">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {TIME_OPTIONS.map(opt => (
                  <button key={opt.value} onClick={() => { setTimeAvailable(opt.value); setStep('drive') }}
                    ...
                    <div className="font-bold text-white text-lg group-hover:text-yd-orange transition-colors">{opt.label}</div>
          ...
          {step === 'drive' && (
            <div>
              <button onClick={() => setStep('time')} className="text-white/40 hover:text-white/70 text-xs mb-4 flex items-center gap-1">← Back</button>
              <p className="text-white/60 text-sm mb-1">
                {"You've got "}
                <span className="text-white font-semibold">
                  {timeAvailable >= 60 ? `${timeAvailable / 60} hr${timeAvailable > 60 ? 's' : ''}` : `${timeAvailable} min`}
                </span>.
              </p>
              <p className="text-white/60 text-sm mb-5">How far are you willing to drive?</p>
              <div className="grid grid-cols-3 gap-3">
                {DRIVE_OPTIONS.map(opt => (
                  <button key={opt.value} onClick={() => { setMaxDrive(opt.value); setStep('vibe') }}
                    ...
                    <div className="font-bold text-white text-base group-hover:text-yd-orange transition-colors">{opt.label}</div>
                    <div className="text-white/40 text-xs mt-0.5">{opt.sublabel}</div>
                  </button>
                ))}
              </div>
          ...
          {step === 'vibe' && (
            <div>
              <button onClick={() => setStep('drive')} className="text-white/40 hover:text-white/70 text-xs mb-4 flex items-center gap-1">← Back</button>
              {error && (
                <div className="bg-red-500/20 border border-red-500/30 text-red-300 text-xs rounded-xl p-3 mb-4">{error}</div>
              )}
              <p className="text-white/60 text-sm mb-5">What are you in the mood for?</p>
              <div className="grid grid-cols-2 gap-3">
                {VIBE_OPTIONS.map(opt => (
                  <button key={opt.value} onClick={() => { setVibe(opt.value); fetchResults(timeAvailable, maxDrive, opt.value) }}
                    ...
                    <span className="font-semibold text-white text-sm group-hover:text-yd-orange transition-colors">{opt.label}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              {error ? (
                <div className="text-center py-8">
                  ...
                  <button onClick={() => setStep('time')} className="bg-yd-orange hover:bg-amber-500 text-white font-bold px-6 py-2.5 rounded-xl text-sm transition-colors">
                ...
                <>
                  <div className="flex items-center justify-between mb-4">
                    <p className="text-white/60 text-xs">{results.length} thing{results.length !== 1 ? 's' : ''} within {maxDrive} min of you</p>
                    <button onClick={() => setStep('time')} className="text-yd-orange hover:text-amber-400 text-xs font-semibold transition-colors">Search again →</button>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {results.map(r => <ResultCard key={r.id} result={r} onLetsGo={handleLetsGo} />)}
                  </div>
                </>
              )}
            </div>
        ...
        {(step === 'time' || step === 'drive' || step === 'vibe') && (
          <div className="flex justify-center gap-1.5 pb-5 shrink-0">
            {(['time', 'drive', 'vibe'] as const).map(s => (
              <div key={s} className={`w-1.5 h-1.5 rounded-full transition-colors ${step === s ? 'bg-yd-orange' : 'bg-white/20'}`} />
            ))}
          </div>
        )}
      </div>

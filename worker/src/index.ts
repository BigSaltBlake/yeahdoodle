...
const CITIES = process.env.CITIES ? process.env.CITIES.split(',').map(c => c.trim()) : DEFAULT_CITIES
...
function startScheduler() {
  ...
  cron.schedule('0 5 * * *',         async () => { const db = createDb(); await runSync('Cleanup', () => deleteExpiredEvents(db)) })
  console.log('Scheduler running. Cities:', CITIES.join(', '))
  console.log('Schedule: TM 4x/day | SeatGeek 2x/day | AI 1x/day | Google 2x/day | Meetup 2x/day | Cleanup 1x/day')
}

;(async () => {
  console.log('YeahDoodle Worker starting...')
  console.log('Supabase: ' + (process.env.SUPABASE_URL ?? '(not set)'))
  console.log('Ticketmaster: ' + (process.env.TICKETMASTER_API_KEY ? 'set' : 'NOT SET'))
  console.log('SeatGeek: ' + (process.env.SEATGEEK_CLIENT_ID ? 'set' : 'NOT SET'))
  console.log('Anthropic: ' + (process.env.ANTHROPIC_API_KEY ? 'set' : 'NOT SET (enrichment disabled)'))
  await bootSync()
  startScheduler()
})()